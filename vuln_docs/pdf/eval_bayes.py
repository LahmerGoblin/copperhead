...
for res_set in res_sets:
    ...
    gt_keys = [ k[len('pdf-train/'):] for k in gt_keys]
    ...
    for gt_key in gt_keys:
        ...
        if int(reader.groundtruth_dict[gt_key])== int(res_set['pdf-train/'+gt_key][0]):
            if truth == 1:
...
for res_set in res_sets:
    ...
    for gt_key in res_set.keys():
        ...
        transfer_tolist.append(reader.groundtruth_dict[gt_key[len('pdf-train/'):]])
